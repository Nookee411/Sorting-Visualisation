...
export class Sort {
  ...
  async insertionSort() {
    ...
    for (let i = 1; i < this.array.length; i++) {
      ...
      while (
        indexOfEle > 0 &&
        !this.#compareAndDispatch(indexOfEle, indexOfEle - 1) &&
        this.isSorting
      ) {
    ...
    if (this.isSorting) {
      ...
      while (i < leftArr.length && j < rightArr.length && this.isSorting) {
        ...
        await this.sleepDuration(config.ComparisonTime).then(() => {
          ...
          this.dispatch(SortEvent.ItemScanned, {
            index: startIndex + mergedArray.length,
          });
        });
      }
  ...
  async #heapSort() {
    this.isSorting = true;
    await this.makeMaxHeap();
    await this.applyHeapSort();
  }

  async makeMaxHeap() {
    let middle = Math.floor(this.array.length / 2);
    for (let i = middle; i >= 0 && this.isSorting; i--) {
      await this.pushToMaxArray(this.array, i);
    }
  }

  async applyHeapSort() {
    let heapContainer = [...this.array];
    for (let i = 0; heapContainer.length > 0 && this.isSorting; i++) {
      this.array[i] = heapContainer[0]; //Extracting max
      heapContainer[0] = heapContainer[heapContainer.length - 1]; //swap small element to the top
      heapContainer.pop();
      this.pushToMaxArray(heapContainer, 0);
      await this.sleepDuration(config.ComparisonTime);
      this.dispatch(SortEvent.ItemSwapped, { index: i });
    }
  }

  async pushToMaxArray(array, i) {
    ...
    if (
      ...
      this.dispatch(SortEvent.ItemSwapped, { index: maxChildIndex });
      this.pushToMaxArray(array, maxChildIndex);
      await this.sleepDuration(config.SwapTime);
    }
  }
